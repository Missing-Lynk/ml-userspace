...
Composite mode (vs direct tile scanout) is deliberate, not a fallback: it is the only mode that leaves a plane free for the OSD overlay AND produces the assembled 1080p frame the DVR encodes. Recording tees off the composite; the OSD overlay itself is blended at scanout only, so by default the recorded video has **no OSD burned in** (matching the vendor). The SD card mounts at `/mnt/sdcard` (`ml-sdcard` init); recordings and the menu's Playback list both live there.

**BTFL OSD burn-in** (`dvr.record_osd`): with the Record OSD setting on, the recording carries the BTFL OSD (and only it - never the System OSD bar or the menu). The HUD renders each changed OSD cell with its loaded MSP font at the exact on-screen cell rectangle and sends the RGBA patch over `ctrl.sock` (`MLM_CMD_OSD_CELL`); the pipeline converts each patch once to YUV + opaque-pixel spans (`mlp-osdburn.c`) and, while recording, memcpys the spans into every completed composite before it is flushed and encoded. The burned glyphs also reach the panel, where they sit exactly under the overlay plane's identical ones (invisible). No extra CMA/MMZ is spent (the cell cache is plain malloc), the composite is only ever written (binary alpha, no read-back from WC memory), and the per-frame cost is a few hundred small memcpys.

**DVR records native 1080p60**: 60 fps composite display + a playable 1920x1080@60 HEVC MP4 simultaneously, `drop=0 evict=0`. `ML_DVR_RES=720 ML_DVR_FPS=30` selects a smaller recording if ever needed. Recording is H.265 by default (needs the enlarged MMZ carveout kernel); `ML_DVR_CODEC=h264` records H.264 instead; playback probes the MP4 sample entry (`avc1` vs `hvc1`/`hev1`) and picks the matching decoder. Recording robustness: `push_au` bounds each input appsrc at 4 MiB with drop-until-IRAP resync (appsrc `block=false` never fails a push; an unbounded input queue OOMs).
