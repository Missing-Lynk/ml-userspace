/**
 * @file lv_conf.h
 * @brief LVGL's configuration file (v9.2.2), not ours by origin.
 *
 * Copied from upstream lv_conf_template.h and customised for this project: 32-bit colour
 * (LV_COLOR_DEPTH 32) for real alpha so the OSD composites over the live video, the larger
 * Montserrat fonts enabled, and the dark theme as default. The build points LVGL here via
 * LV_CONF_INCLUDE_SIMPLE. Everything else is upstream defaults; this file follows LVGL's own
 * formatting, not libre/CODING_STYLE.md.
 */
...
#define LV_ASSERT_HANDLER abort();   /*Abort (not halt): a failed alloc raises SIGABRT, which ml-hud's
                                       fatal-signal handler catches to silence the buzzer before dying,
                                       instead of spinning forever with the key-tone latched on.*/
