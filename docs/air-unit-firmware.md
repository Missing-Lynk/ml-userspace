...
> A **stock** air unit and a **stock** goggle both enumerate their gadget at `192.168.3.100` (and share the hostname `art_sirius`); only plug in one at a time. `net-up.sh` prints which one answered. Once **flashed** with the open stack the two get distinct per-index addresses (goggle `192.168.3.101`, air `192.168.3.102`), so they no longer collide.
...
- rootfs `/dev/root` = squashfs on `ubiblock0_0`, 15.5M on flash (34.5M decompressed, 629 inodes). `/usrdata` ubifs (rw) holds `lowdelay/` config; `/factory` ubifs holds `certification`, a per-unit token the stock media stack verifies at startup. The open stack does not use or mount it; never write that partition (see the flashable-image guide).
