...
- **Records are LENGTH-CHAINED** [confirmed on-device]. The byte immediately BEFORE each `b6 03` marker is that record's declared length. A record is `b6 03 <row> <col> <attr> <glyphs...> <next_len>`, and its **last byte is the length of the FOLLOWING record** (so it doubles as the next record's prefix). The displayable glyphs are content bytes `3 .. len-2` (skip row/col/attr and the trailing next_len).

  **The declared length is uniformly `4 + glyph_count`**: 3 for row/col/attr, plus the glyphs, plus one for the chain byte. The final record is **not** an exception, even though it carries no chain byte: the vendor still declares `4 + glyph_count` there, so the last record's declared length is one MORE than the bytes that actually follow its marker. Measured across the vendor capture, e.g. a final record of 6 glyphs (`DISARM`) is declared `10`, not `9`.

  This is worth stating flatly because an earlier wording here said the content "is `len` bytes", which is true for every record except the last and produced an off-by-one in our encoder that a vendor goggle would have failed to parse. Our own decoder never noticed, because `msp_canvas.c` ignores the declared length entirely and rescans for `b6 03`. Guarded by `userspace/tests/msp-canvas-roundtrip.c` (`make check`), which re-encodes captured vendor canvases and requires byte equality.
