...
| 720p60 | `videoscale` to 1280x720 | 60 | no | CPU scale; the `ar_scaler` HW path exists at the driver level but is not wired into the record bin |
| 720p30 | `videoscale` to 1280x720 | 30 | no | lightest encoder load; scale is CPU-bound |

Downscale breaks zero-copy (the v4l2 encoder can't scale, so `videoscale` copies to system memory first). Frame-rate reduction stays zero-copy for the kept frames.

**HW downscale is available from the kernel** (validated 2026-07-17): `ar_scaler` accepts dmabuf fds directly via `SCALER_IOC_BATCH_DMABUF` (`kernel/modules/ar_scaler.h`) - a 3-plane I420 1920x1080 -> 1280x720 batch runs in ~6 ms/frame (166 fps ceiling), keeping the record path dma-buf-to-dma-buf (composite in, scaled pool buffer out, `dmabuf-import` into the encoder). Integration into `mlp-record.c` and one kernel prerequisite (dropping the scaler's `ar_osal` dependency) are pending: `plans/hw-downscaled-dvr-scaler.md`.
