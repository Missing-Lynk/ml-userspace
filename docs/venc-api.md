...
|-------|-------------------|-------------|--------|
| H.264 | `v4l2h264enc` | `V4L2_PIX_FMT_H264` | enumerates; validated end-to-end (playable MP4). `ML_DVR_CODEC=h264` selects it |
| H.265 / HEVC | `v4l2h265enc` | `V4L2_PIX_FMT_HEVC` | enumerates; validated end-to-end at 1080p60 (playable MP4). DVR default |
...
- **Memory - the encoder's CMA appetite (two over-allocations, both now addressed).** The encoder's v4l2 vb2 pools come from **CMA** (`CmaTotal 64 MiB`, carved from only 148 MiB System RAM; the wave5 decoder's frames use the separate `no-map` MMZ pool (the device DTS mmz reserved-memory node), which vb2 MMAP buffers cannot). Two over-allocations blew CMA:
